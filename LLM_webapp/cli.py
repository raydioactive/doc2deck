...
def get_api_key(specified_key=None):
    """Get API key from argument or environment variable."""
    api_key = specified_key
    if not api_key:
...
def print_models(provider: str, models: List[Dict[str, Any]]):
    """Print models in a formatted table based on provider."""
...
def run_list_models_command(args):
    ...
    print(f"\nFetching available models for {args.provider.capitalize()}...")
    models = get_available_models(args.provider, api_key)
    
    if not models:
        print(f"No models found for {args.provider} or API error occurred.")
        return 1
        
    # Print the models
    print_models(args.provider, models)
    return 0
...
def setup_parser():
    ...
    parser.add_argument('--provider', choices=['claude', 'gemini', 'openai'], help='LLM provider for list-models')
    parser.add_argument('--api-key', help='API key (reads LLM_API_KEY env var if not provided)')
...
def run_cli():
    ...
    if not args.command:
        if args.list_models and args.provider:
            # Create args structure similar to list-models subcommand
            class ListArgs:
                def __init__(self, provider, api_key):
                    self.provider = provider
                    self.api_key = api_key
            
            list_args = ListArgs(args.provider, args.api_key)
            return run_list_models_command(list_args)
    ...
    if args.command == 'list-models':
        return run_list_models_command(args)
